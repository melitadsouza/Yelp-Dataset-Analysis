...
def generateCSV(year, count = None):
    ...
    DESTINATION = "D:\\YELP Dataset\\wordcloud datasets\\yelp_restaurants_reviews_nouns_{}.csv".format(year);
    
    bus_rest = pd.read_csv("D:\\YELP Dataset\\tobesubmitted\\yelp_academic_dataset_business_restaurants.csv", low_memory=False);
    
    with open("D:\\YELP Dataset\\restaurant related full datasets\\restaurants_businessIds.txt", 'r') as busIds:
        id_list = busIds.read();
